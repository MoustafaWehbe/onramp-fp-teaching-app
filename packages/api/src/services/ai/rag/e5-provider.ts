...
export const E5_MODEL_REVISION = "614241f622f53c4eeff9890bdc4f31cfecc418b3";
export const E5_INFERENCE_LIBRARY_VERSION = "4.2.0";
export const E5_EMBEDDING_DIMENSION = 384;
export const E5_MODEL_VERSION = `hf:${E5_MODEL_REVISION}|transformers.js:${E5_INFERENCE_LIBRARY_VERSION}|fp32|mean|l2:v1`;
...
async function loadTransformersPipeline(): Promise<FeatureExtractor> {
  try {
    ...
    const extractor = await transformers.pipeline(
      "feature-extraction",
      E5_MODEL_ID,
      { revision: E5_MODEL_REVISION, dtype: "fp32" },
    );
...
function outputRows(output: unknown): number[][] {
  const candidate = output as { tolist?: () => unknown };
...
export class MultilingualE5Provider implements EmbeddingProvider {
  readonly modelId = E5_MODEL_ID;
  readonly modelVersion = E5_MODEL_VERSION;
  readonly dimension = E5_EMBEDDING_DIMENSION;

  private readonly batchSize: number;
  private readonly pipelineLoader: E5PipelineLoader;
  private pipelinePromise: Promise<FeatureExtractor> | undefined;

  constructor(options: E5EmbeddingProviderOptions = {}) {
    this.batchSize = boundedBatchSize(
      options.batchSize ?? configuredBatchSize(),
    );
    this.pipelineLoader = options.pipelineLoader ?? loadTransformersPipeline;
  }
  ...
  private async embedFormatted(inputs: readonly string[]): Promise<number[][]> {
    ...
    for (let offset = 0; offset < inputs.length; offset += this.batchSize) {
      ...
      try {
        ...
        vectors.push(
          ...rows.map((row) => normalizeEmbedding(row, this.dimension)),
        );
      } catch (error: unknown) {
        if (error instanceof EmbeddingInferenceError) throw error;
        throw new EmbeddingInferenceError(
  ...
  async embedPassage(content: string): Promise<number[]> {
    const [vector] = await this.embedPassages([content]);
    if (!vector) {
      throw new EmbeddingInferenceError("Passage embedding returned no vector");
    }
    return vector;
  }
  ...
  async embedQuery(query: string): Promise<number[]> {
    ...
    if (!vector) {
      throw new EmbeddingInferenceError("Query embedding returned no vector");
    }
    return vector;
  }
