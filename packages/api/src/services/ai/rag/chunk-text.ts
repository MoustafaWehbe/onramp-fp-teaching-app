...
function avoidsSplittingSurrogatePair(content: string, offset: number): number {
...
function trimStart(
  ...
  while (offset < endOffset && isWhitespace(content[offset])) offset += 1;
  return offset;
}
...
function trimEnd(
  ...
  while (offset > startOffset && isWhitespace(content[offset - 1])) offset -= 1;
  return offset;
}
...
function preferredEndOffset(
  ...
  for (let offset = hardEnd - 1; offset >= minimumBoundary; offset -= 1) {
    if (isWhitespace(content[offset])) return offset;
  }
...
function isStartBoundary(
  ...
  if (boundary === "line") return content[offset - 1] === "\n";
...
function preferredNextStart(
  ...
  if (overlap === 0) return previousEnd;

  const idealOffset = Math.max(previousStart + 1, previousEnd - overlap);
  ...
  for (const boundary of ["paragraph", "line", "whitespace"] as const) {
...
export function chunkText(
  ...
  const targetSize = options.targetSize ?? DOCUMENT_CHUNK_TARGET_SIZE;
  ...
  while (cursor < content.length) {
    ...
    if (rawEnd >= content.length) break;
    ...
    if (nextCursor <= cursor) {
      nextCursor = Math.min(rawEnd, cursor + Math.max(1, targetSize - overlap));
    }
    cursor = nextCursor;
  }
