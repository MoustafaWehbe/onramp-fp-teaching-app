...
export function LessonEditorDialog({
  ...
  useEffect(() => { if (open) { setTitle(lesson?.title ?? ""); setContent(lesson?.content ?? ""); setVideoUrl(lesson?.videoUrl ?? ""); setStarterCodeUrl(lesson?.starterCodeUrl ?? ""); setOrder(String(lesson?.order ?? 0)); setModuleId(lesson?.moduleId ?? modules[0]?.id ?? ""); } }, [open, lesson, modules]);
...
export function DeleteConfirmationDialog({ open, entityName, isDeleting, error, onCancel, onConfirm }: { open: boolean; entityName: string; isDeleting: boolean; error: unknown; onCancel: () => void; onConfirm: () => void; }) {
  return <Dialog open={open} title={`Delete ${entityName}`}><div className="mt-5 space-y-4"><p className="text-sm text-muted-foreground">Delete <strong className="text-foreground">{entityName}</strong>? This action cannot be undone.</p><ErrorMessage error={error} /><div className="flex justify-end gap-2"><Button type="button" variant="outline" onClick={onCancel} disabled={isDeleting}>Cancel</Button><Button type="button" variant="destructive" onClick={onConfirm} disabled={isDeleting}>{isDeleting ? "Deleting..." : "Delete"}</Button></div></div></Dialog>;
}
